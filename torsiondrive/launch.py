...
import shutil
import logging
from .misc import EnhancedYamlFormatter

# Initialize a logger for the application using a custom YAML formatter.
# This setup enhances readability and usability of log messages, which is crucial for debugging and monitoring the software's execution.
logger = logging.getLogger("yamlLogger")

# Create a FileHandler to write logs to a file
log_file_path = "torsiondrive.log.yaml"  # Specify the path to your log file
...
def load_dihedralfile(dihedralfile, zero_based_numbering=False):
    ...
    assert all(
        low >= -180 and high <= 360 and low < high for low, high in dihedral_ranges
    ...
    if dihedral_ranges != []:
        assert len(dihedral_idxs) == len(
            dihedral_ranges
        ), f"Dihedral ranges {dihedral_ranges} length not consistent with dihedral idxs {dihedral_idxs}"
...
def create_engine(enginename, inputfile=None, work_queue_port=None, native_opt=False, extra_constraints=None):
    """Create a QM Engine object with work_queue and geomeTRIC setup.

    Parameters
    ----------
    enginename : str
        Name of compute engine plugin.
    inputfile : str, optional
        Input file path.
    work_queue_port : int, optional
        Port to connect to existing work queue manager.
    native_opt : bool, optional
        If True, will not use geometric.optimize() wrapper. Default False.
    extra_constraints : list, optional
        Extra optimization constraints.

    Returns
    -------
    engine : QCEngine compute engine instance
        Instance of QCEngine compute engine class.

    Notes
    -----
    This function is intentionally left outside of DihedralScanner class, because multiple DihedralScanner could share the same engine
    """
    engine_dict = {
        "ase": EngineASE,
        "xtb": EnginexTB,
    ...
    if enginename == "gaussian":
        ...
        engine = engine_dict[enginename](
            inputfile, work_queue, native_opt=native_opt, extra_constraints=extra_constraints
        )
    return engine
...
def run(
    ...
    if zero_based_numbering is True:
        print(
            "The use of command line --zero_based_numbering is deprecated and will be removed in the future. Please use #zero_based_numbering in dihedralfile"
        )
    ...
    if n_grid_spacing == grid_dim:
        ...
        raise ValueError(
            "Number of grid_spacing values %d is not consistent with number of dihedral angles %d"
            % (grid_dim, n_grid_spacing)
        )

    # create QM Engine, and WorkQueue object if provided port
    engine = create_engine(engine, inputfile=inputfile, work_queue_port=work_queue_port, native_opt=native_opt)

    # load init_coords if provided
    init_coords_M = Molecule(init_coords) if init_coords else None

    # create DihedralScanner object
    scanner = DihedralScanner(
        engine,
    ...
    if verbose:
        ...
        for grid_id in sorted(scanner.grid_energies.keys()):
            print("  {:^20} {:.10f}".format(str(grid_id), scanner.grid_energies[grid_id]))
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Potential energy scan of dihedral angle from 1 to 360 degree",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "inputfile",
        type=str,
        help="Input template file for QMEngine. Geometry will be used as starting point for scanning.",
    )
    parser.add_argument("dihedralfile", type=str, help="File defining all dihedral angles to be scanned.")
    parser.add_argument(
        "--init_coords",
        type=str,
        help="File contain a list of geometries, that will be used as multiple starting points, overwriting the geometry in input file.",
    )
    parser.add_argument(
        "-g",
        "--grid_spacing",
        ...
        nargs="*",
        default=[15],
        help="Grid spacing for dihedral scan, i.e. every 15 degrees, multiple values will be mapped to each dihedral angle",
    )
    parser.add_argument(
        "-e",
        "--engine",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Provide a constraints file in geomeTRIC format for additional freeze or set constraints (geomeTRIC or TeraChem only)",
    )
    parser.add_argument(
        "--native_opt",
        action="store_true",
        default=False,
        help="Use QM program native constrained optimization algorithm. This will turn off geomeTRIC package.",
    )
    parser.add_argument(
        "--energy_thresh",
        type=float,
        default=1e-5,
        help="Only activate grid points if the new optimization is <thre> lower than the previous lowest energy (in a.u.).",
    )
    parser.add_argument(
        "--energy_upper_limit",
        type=float,
        default=None,
        help="Only activate grid points if the new optimization is less than <thre> higher than the global lowest energy (in a.u.).",
    )
    parser.add_argument(
        "--wq_port",
        type=int,
        default=None,
        help="Specify port number to use Work Queue to distribute optimization jobs.",
    )
    parser.add_argument(
        "--zero_based_numbering", action="store_true", help="Use zero_based_numbering in dihedrals file."
    )
    parser.add_argument(
    ...
    run(
        ...
        dihedralfile=args.dihedralfile,
